...
// `copilot plugin install` or repo-local hook setup even when the plugin
...
const installedMarker = path.join(pluginRoot, "node_modules", ".armorcopilot-installed");
const packageFiles = [
...
if (!installedOk()) {
  process.stderr.write("[armorcopilot] installing dependencies (one-time)...\n");
  const result = spawnSync("npm", ["install", "--omit=dev", "--silent", "--no-audit", "--no-fund"], {
    cwd: pluginRoot,
    stdio: ["ignore", "ignore", "inherit"]
  });
  if (result.status !== 0) {
    process.stderr.write("[armorcopilot] npm install failed (exit " + result.status + ")\n");
    process.exit(1);
  }
  try {
    const pkg = JSON.parse(
...
// MCP servers and hook routers communicate with Copilot via JSON-RPC / JSON
// over stdio. Any non-JSON write to stdout corrupts the protocol and Copilot
...
if (target === "router") {
  ...
  process.stderr.write("[armorcopilot] bootstrap: unknown target '" + target + "'\n");
  process.exit(2);
}
