...
/**
 * Some MCP clients (and Copilot itself sometimes pass complex tool arguments
...
async function run() {
  ...
  server.registerTool(
    ...
    {
      title: "Policy Update",
      description: "Manage ArmorCopilot policy rules (update/list/delete/reset)",
      inputSchema: {
    ...
    async (args) => {
      ...
      if (!config.policyUpdateEnabled) {
        return toTextResult("ArmorCopilot policy updates are disabled.");
      }
  ...
  server.registerTool(
    ...
    {
      title: "Policy Read",
      description: "Read current ArmorCopilot policy state",
      inputSchema: {
  ...
  server.registerTool(
    ...
    async (args) => {
      // Copilot sometimes serializes complex tool arguments as JSON strings
          ...
          try {
            ...
            if (result?.tokenRaw) {
              ...
              process.stderr.write(
                `[armorcopilot] backend token issued, tokenLen=${result.tokenRaw.length}\n`
              );
            }
          } catch (err) {
            const msg = err instanceof Error ? err.message : String(err);
            process.stderr.write(`[armorcopilot] intent capture failed: ${msg}\n`);
          }
        })();
      }
  ...
  const flusher = setInterval(async () => {
    try {
      ...
      process.stderr.write(
        `[armorcopilot-policy] flushed ${rows.length} audit rows -> offset=${endOffset}\n`
      );
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      process.stderr.write(`[armorcopilot-policy] flusher: ${msg}\n`);
    }
  }, 5000);
  flusher.unref?.();
...
run().catch((error) => {
  ...
  process.stderr.write(`[armorcopilot-policy] ${message}\n`);
  process.exitCode = 1;
});
