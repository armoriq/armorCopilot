 ...
 *  - register_intent_plan MCP tool (validates Copilot's input)
 *  - register_intent_plan inputSchema (model sees this when invoking the tool)
 *
 * Copilot has no ExitPlanMode-equivalent event, so unlike ArmorClaude there is
 * no plan-file extraction path on Copilot.
 */
...
/**
 * Human-readable format string injected into Copilot's context so it knows
