...
export function evaluatePolicy({ policy, toolName, toolParams }) {
  ...
  for (const rule of rules) {
    ...
    if (rule.action === "deny") {
      return {
        allowed: false,
        reason: `ArmorCopilot policy deny: ${rule.id}`,
        matchedRule: rule,
    ...
    if (rule.action === "require_approval") {
      return {
        allowed: false,
        reason: `ArmorCopilot policy requires approval: ${rule.id}`,
        matchedRule: rule,
        dataClasses: Array.from(dataClasses),
        warnings
      };
...
// Real Copilot tools we recognize. Used to disambiguate "block X for Y" where X
// may or may not be a tool name. Falls back to "*" when X isn't here.
const KNOWN_COPILOT_TOOLS = new Set([
  "*",
  "bash", "shell", "write", "read", "edit", "apply_patch", "list_dir", "view_image", "mcp_resource", "websearch", "fetch",
...
function buildPolicyUpdateFromText(text, state, forceNewId = false) {
  ...
  if (anyParam && tool !== "*") {
    ...
    if (!KNOWN_COPILOT_TOOLS.has(normalized)) {
      tool = "*";
    }
  }
