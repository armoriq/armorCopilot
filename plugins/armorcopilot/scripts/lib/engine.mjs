...
function isPolicyUpdateAllowed(config, input) {
  if (!config.policyUpdateEnabled) {
    return { allowed: false, reason: "ArmorCopilot policy updates disabled" };
  }
  ...
  return allowed
    ...
    : {
        allowed: false,
        reason: "ArmorCopilot policy update denied",
        candidates
      };
}
...
export async function handleUserPromptSubmit(input, config) {
  ...
  if (policyCommandLooksLikePrompt(prompt)) {
    ...
    if (!allowed.allowed) {
      return blockPrompt(allowed.reason || "ArmorCopilot policy update denied");
    }
...
export async function handlePreToolUse(input, config) {
  ...
  if (!toolName) {
    ...
    return denyOrAllow(config, "ArmorCopilot: missing tool_name on PreToolUse");
  }

  // --- Whitelist: ArmorCopilot's own MCP tools must never be blocked,
  //     otherwise the agent can't register a plan or read/update policy.
  //     Match the exact MCP prefix from .mcp.json (armorcopilot-policy),
  ...
  // Copilot MCP namespace is `mcp__<server>__` and the underlying MCP server name
  // can carry hyphens (`armorcopilot-policy`) or be sanitized to underscores
  // (`armorcopilot_policy`). Copilot's TUI display also surfaces `<server>.<tool>`
  ...
  const ARMOR_SERVER_RE = /(mcp__armorcopilot[-_]policy__|armorcopilot[-_]policy[._])/;
  if (
    armorTools.some(
      (t) =>
        norm === t ||
  ...
  // it when Copilot has registered a NEW plan, and stale plans must be
  ...
  const runtimeState = await loadRuntimeState(config.runtimeFile);
  // Per-session plan file so concurrent Copilot windows don't clobber each
  ...
  if (config.cryptoPolicyEnabled) {
    try {
      ...
      if (cachedState?.policyDigest) {
        ...
        if (!check.valid) {
          return denyOrAllow(config, `ArmorCopilot crypto policy mismatch: ${check.reason}`);
        }
      }
    } catch (error) {
  ...
  if (!policyDecision.allowed) {
    return denyPreTool(policyDecision.reason || "ArmorCopilot policy denied");
  }
  ...
  if (
    ...
    try {
      ...
      const refreshed = await requestIntent(config, {
        ...
        metadata: { source: "copilot", trigger: "auto_refresh" }
      });
  ...
  if (!intentTokenRaw && (config.intentEndpoint || (config.useSdkIntent && config.apiKey))) {
    try {
      ...
      if (config.intentRequired && shouldDeny(config)) {
        return denyPreTool(`ArmorCopilot intent planning failed: ${message}`);
      }
    }
  }
  ...
  if (intentTokenRaw && config.verifyStepEndpoint && config.csrgVerifyEnabled) {
    try {
      ...
      if (verifyResult.allowed === false) {
        return denyOrAllow(
          config,
          verifyResult.reason || `ArmorCopilot intent verification denied for ${toolName}`
        );
      }
      ...
      const message = error instanceof Error ? error.message : String(error);
      const deny = denyOrAllow(config, `ArmorCopilot verify-step failed: ${message}`);
      if (deny) {
        return deny;
      }
    }
  ...
  if (Number.isFinite(localExpiresAt) && nowEpochSeconds() > localExpiresAt) {
    const deny = denyOrAllow(
      config,
      "ArmorCopilot intent token expired — call register_intent_plan with your current plan to refresh, then retry the tool"
    );
    if (deny) {
      return deny;
    }
  ...
  if (!intentTokenRaw && localPlan && typeof localPlan === "object") {
    ...
    if (localCheck.allowed) {
      ...
      const deny = denyOrAllow(config, localCheck.reason || "ArmorCopilot intent drift");
      if (deny) {
        return deny;
      }
    }
  ...
  if (config.intentRequired && !remoteAllowed && !tokenCheckMatched && !localPlanMatched) {
    const deny = denyOrAllow(config, "ArmorCopilot intent plan missing for this session");
    if (deny) {
      return deny;
    }
  }
...
export async function handlePermissionRequest(input, config) {
  ...
  if (!policyDecision.allowed && shouldDeny(config)) {
    return denyPermissionRequest(policyDecision.reason || "ArmorCopilot policy denied approval request");
  }
