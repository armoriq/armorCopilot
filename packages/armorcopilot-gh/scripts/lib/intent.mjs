...
function buildFallbackPlan(payload) {
  const goal = typeof payload.prompt === "string" ? payload.prompt : "ArmorCopilot intent";
  const plan = { steps: [], metadata: { goal, source: "copilot" } };
  if (typeof payload.toolName === "string" && payload.toolName.trim()) {
    plan.steps.push({
      action: payload.toolName.trim(),
      mcp: payload.mcpName || "copilot",
...
export function checkToolAgainstPlan({ plan, toolName, toolInput }) {
  ...
  const steps = Array.isArray(plan?.steps) ? plan.steps : [];
  if (!steps.length) {
    return { allowed: false, reason: "ArmorCopilot intent plan is empty" };
  }
  const matches = [];
  for (const step of steps) {
    if (!isPlainObject(step)) {
  ...
  if (!matches.length) {
    return { allowed: false, reason: `ArmorCopilot intent drift: tool not in plan (${toolName})` };
  }
  ...
  for (const step of matches) {
    ...
    for (const candidate of inputCandidates) {
      ...
      // of Copilot's `command`). If NONE of the declared keys exist on the
  ...
  if (sawConstrainedMatch) {
    return {
      allowed: false,
      reason: `ArmorCopilot intent mismatch: parameters not allowed for ${toolName}`
    };
  }
  return { allowed: true };
}
