 ...
 *  2. No plan mode: Copilot calls register_intent_plan MCP tool directly
 *     (handled in policy-mcp.mjs, not here)
 *
 * This module handles only PARSING — plan generation is done by Copilot's own
 ...
 * The UserPromptSubmit directive tells Copilot to include the plan as a
 ...
 * This is retained for compatibility with imported tests and future Copilot
 * plan-file events; current Copilot hooks do not expose ExitPlanMode.
 */
 ...
 * Resolve a best-effort Copilot-scoped plan path.
 */
export function resolvePlanFilePath(input) {
  ...
  if (sessionName) {
    ...
    return `${homeDir}/.copilot/plans/${sessionName}.md`;
  }
